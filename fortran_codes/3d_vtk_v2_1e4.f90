...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  partition_number = 1
  allocate(partition_kstart(1:partition_number))
  allocate(partition_kend(1:partition_number))

  partition_length_last = mod(kend-kstart+1,partition_number)
  partition_length = (kend-kstart+1-partition_length_last)/partition_number
  
  write(6,*) "kstart, kend = ", kstart,kend
  write(6,*) "partition_length, partition_length_last = ", partition_length, partition_length_last

  partition_kstart(1) = kstart
  partition_kend(1)   = partition_kstart(1) + partition_length
  write(6,*) "i,kstart(i),kend(i) = ", 1,partition_kstart(1),partition_kend(1)

  do i = 2, partition_number-1
     partition_kstart(i) = partition_kend(i-1)+1
     partition_kend(i)   = partition_kstart(i) + partition_length
     write(6,*) "i,kstart(i),kend(i) = ", i,partition_kstart(i),partition_kend(i)
  enddo

  if(partition_length_last .ne. 0) then
     partition_kstart(partition_number) = partition_kend(partition_number-1)+1
     partition_kend(partition_number)   = partition_kstart(partition_number) + partition_length
     write(6,*) "i,kstart(i),kend(i) = ", partition_number,partition_kstart(partition_number),partition_kend(partition_number)
  endif
  i = 1
  !do i = 1, partition_number-1
     ! write(filename,'(a,i3.3,a)') "/media/SS-250GB/1E4_FINF/Q_1E4_INF_PART", &
     !                                  i,".vtk"
     ! write(filename,'(a,i3.3,a)') "/media/SS-250GB/1E4_F1/Q_1E4_1_PART", &
     !                                  i,".vtk"
     !write(filename,'(a,i3.3,a)') "/media/BU_HDD_5_MYBOOK8TB/1e4_3_new/run/for_senate/Q_1E4_3_PART", &
     !                                 i,".vtk"
     ...
     !call write_plt_3d_1var(filename,Q,xc,yc,zcg,nx_start,nx_end,ny_start,ny_end,nz_start,nz_end,nx,ny,nz)
    !call write_vtk(filename,Q,nx,ny,nz,xc,yc,zcg,partition_kstart(i),partition_kend(i))
  !enddo
...
!                                      i,".vtk"
!     write(6,*) "START WRITING filename = ", filename
!     call write_vtk(filename,Q,nx,ny,nz,xc,yc,zcg, &
...
subroutine read_grid(grid_dir,xu,yv,zw,zwg,xc,yc,zc,zcg,nx,ny,nz,nzg,ru,rp,tag)
  implicit none
...
subroutine center_velocity(nx,ny,nz,Ucen,Uin,dir)
  ...
  elseif (dir.EQ.3) then 
     ...
     !Invalid direction
     !write(*,'(a60,i2)') "IN34LID DIRECTION IN center_velocities 
  ...
  DO k = nz_start, nz_end
     DO i = nx_start, nx_end
        DO j = ny_start, ny_end
           ...
           xc_3d(i+1-nx_start,j+1-ny_start,k+1-nz_start) = xc(i)*cos(yc(j))
           yc_3d(i+1-nx_start,j+1-ny_start,k+1-nz_start) = xc(i)*sin(yc(j))
